...
import { Paperclip, Trash2, Send, MessageSquare, ThumbsUp, Eye, Stamp } from 'lucide-react';
            ...
            <CardHeader>
                 <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                     ...
                     <Badge variant={communique.status === 'approved' ? 'default' : 'secondary'} className={statusColor[communique.status]}>
                        {statusText[communique.status]}
                     </Badge>
                </div>
            </CardHeader>
            <CardContent className="space-y-4">
                 <div className="relative">
                    ...
                    <div 
                        ...
                        style={{ fontFamily: `var(--font-${communique.content.match(/font-family: ([^;]+);/)?.[1] || 'sans'})`}}
                        dangerouslySetInnerHTML={{ __html: communique.content }} 
                    />
                     {communique.status === 'approved' && (
                        <div className="absolute bottom-0 right-0 flex items-end gap-2 opacity-80">
                            {communique.signatureUrl && <Image src={communique.signatureUrl} alt="Signature" width={120} height={40} data-ai-hint="signature" />}
                            {communique.sealUrl && <Image src={communique.sealUrl} alt="Sceau de l'école" width={70} height={70} data-ai-hint="school seal" />}
                        </div>
                    )}
                 </div>
            ...
            <CardFooter className="py-3 px-6 flex justify-between">
                <div className="flex items-center gap-4 text-muted-foreground">
                 ...
                 <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Stamp className="h-4 w-4" /> <span>Approuvé par: {communique.approvedBy || 'N/A'}</span>
                 </div>
            </CardFooter>
            <Separator />
